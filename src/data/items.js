...
function bonusesForType(type, scale) {
  switch (type) {
    case 'weapon': {
      ...
      if (Math.random() < 0.2) b.lifesteal = parseFloat((rand(0.02, 0.05)  * scale).toFixed(3));
      return b;
    }
    case 'armor': {
      const b = { hp: parseFloat((rand(0.07, 0.11) * scale).toFixed(3)) };
      if (Math.random() < 0.5) b.def    = parseFloat((rand(0.03, 0.06) * scale).toFixed(3));
      if (Math.random() < 0.25) b.thorns = parseFloat((rand(0.02, 0.05) * scale).toFixed(3));
      return b;
    }
    case 'accessory': {
      const r = Math.random();
      if (r < 0.20) return { spd:      parseFloat((rand(0.03, 0.06) * scale).toFixed(3)) };
      if (r < 0.40) return { crit:     parseFloat((rand(0.02, 0.04) * scale).toFixed(3)),
                             critDmg:  parseFloat((rand(0.04, 0.08) * scale).toFixed(3)) };
      if (r < 0.55) return { xpMult:   parseFloat((rand(0.05, 0.10) * scale).toFixed(3)) };
      if (r < 0.70) return { goldMult: parseFloat((rand(0.05, 0.10) * scale).toFixed(3)) };
      if (r < 0.85) return { dodge:    parseFloat((rand(0.03, 0.07) * scale).toFixed(3)) };
      return               { lifesteal:parseFloat((rand(0.02, 0.05) * scale).toFixed(3)),
                             thorns:   parseFloat((rand(0.02, 0.04) * scale).toFixed(3)) };
    }
  }
}
...
export function formatBonuses(bonuses) {
  const LABELS = {
    ...
    dodge: 'УВОРОТ', lifesteal: 'ВАМПИР', thorns: 'ШИПЫ',
  };
