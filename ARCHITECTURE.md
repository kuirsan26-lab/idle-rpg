...
| Стат | База | Рост/ур. | Источники бонуса |
|------|------|-----------|-----------------|
...
| CRIT | 5% | — | класс, апгрейды, экипировка |
| CRITDMG | 150% | — | класс, апгрейды, экипировка |

**Запланированы (см. бэклог — геймплей):**

| Стат | База | Cap | Описание | Источники |
|------|------|-----|----------|-----------|
| DODGE | 0% | 75% | шанс полностью уклониться от удара моба | класс (rogue-ветка), апгрейды, аксессуары |
| LIFESTEAL | 0% | — | % нанесённого урона восстанавливает HP | класс (bloodthirst-ветка), апгрейды, оружие |
| THORNS | 0% | — | % полученного урона отражается обратно мобу | класс (warrior-ветка), апгрейды, броня |

**Архитектура новых статов** — `getStats()` в `GameState.js` возвращает их наравне с остальными; `Combat.js` проверяет при каждом тике урона:
```
onPlayerHit: if dodge → пропустить takeDamage()
onPlayerAttack: if lifesteal → currentHp += dmg * lifesteal%
onPlayerHit: if thorns → mob.hp -= dmg * thorns%
```
...
- [ ] 🔜 **Уворот (dodge)** — шанс уклонения от удара; добавить стат в `getStats()`, проверку в `Combat.js` перед `takeDamage()`; апгрейд (+2%/ур.); бонус у rogue-ветки и в аксессуарах; cap 75%; анимация «miss» в BattleStrip
- [ ] 🔜 **Вампиризм (lifesteal)** — % нанесённого урона восстанавливает HP; heal сразу после расчёта `dmg` в `Combat.js`; апгрейд (+3%/ур.); бонус у `bloodthirst`/`vampire`/`bloodlord` и в оружии; не может переполнить maxHp
- [ ] 🔜 **Шипы (thorns)** — % входящего урона отражается мобу; применяется после `takeDamage()`, может добить моба; апгрейд (+3%/ур.); бонус у warrior/paladin-ветки и в броне; синергирует с несколькими мобами в волне
- [ ] 🔜 **Флаги мобов** — механические архетипы в `mobs.js`: `shield` (поглощает N урона), `regen` (восстанавливает HP каждый тик), `armored` (DEF×1.5, уязвим к критам), `swift` (скорость×2, HP×0.5); боссы (`wave % 10`) гарантированно получают 1–2 флага; флаги рендерятся иконками в чипах BattleStrip
- [ ] 🔜 **Многоударность (multi-strike)** — бонус класса `multiStrike: N`; при атаке бьёт `N` первых мобов вместо одного; lifesteal применяется к суммарному урону; синергирует с rogue/archer-веткой; реализуется в `Combat.js` заменой `mobs[0]` на цикл по `mobs.slice(0, N)`
