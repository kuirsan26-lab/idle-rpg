...
src/
  ...
  ui/
    ...
    ClassTreeGraph.js   — горизонтальная DOM-таблица классов (depth 1–5 + lazy 6–10); фильтры Все/Открытые/Доступные; инфо-панель по клику; пульс-кнопка в HUD
    BattleInfoPanel.js  — левая панель арены: зона, прогресс волны, список врагов+HP, счётчик убийств
    Tutorial.js         — онбординг первого запуска: 4 шага с подсветкой целей; state.tutorialDone сохраняется между ранами
...
├────────────────────────────────────────────────────────────────────────────┤
│  #battle-info-panel(160px) · #game-container · Phaser 620×480 · #stats-panel(300px) │
│  зона/враги/убийства         центрирован flexbox              статы + апгрейды      │
├────────────────────────────────────────────────────────────────────────────┤
│                          #combat-log (96px)                                 │
└────────────────────────────────────────────────────────────────────────────┘
...
- **v2.1.0:** `ClassTreeGraph.js` полностью переписан — радиальный граф заменён горизонтальной DOM-таблицей. Depth 1–5 рендерятся сразу, depth 6–10 лениво по кнопке. Три фильтра: Все / Открытые (предки+текущий) / Доступные (прямые дети). CSS-классы: `cls-current`, `cls-ancestor`, `cls-available`, `cls-locked`, `cls-prestige`. Инфо-панель по клику сохранена.
- Неизвестные ноды: `opacity: 0.35`, `cls-locked` класс
...
- [x] **Visual Polish (v2.1.0):** zoom камеры ×1.6 (крупнее спрайты), цветной floating damage (⚡крит/☠яд/🔥горение), hit-flash спрайтов, death tween мобов, градиент body
- [x] **BattleInfoPanel (v2.1.0):** левая панель `#battle-info-panel (160px)` — зона, волна, враги+HP, счётчик убийств (`ui/BattleInfoPanel.js`)
- [x] **Фильтры дерева классов (v2.1.0):** тоглы Все/Открытые/Доступные в `ClassTreeGraph.js`
- [x] **Tutorial (v2.1.0):** `ui/Tutorial.js` — 4 шага онбординга, `state.tutorialDone` не сбрасывается при hardReset
- [x] **Level Up FX (v2.1.0):** Phaser-кольцо от героя + DOM-баннер 1.5с
- [x] **Zone Complete overlay (v2.1.0):** `#zone-complete-overlay` после победы над боссом зоны → ZoneMap
- [x] **HUD cleanup (v2.1.0):** Души+Зеркало → одна кнопка; `margin-left:auto` разделяет инфо/действия/утилиты; XP-текст скрыт
