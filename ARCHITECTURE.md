...
- [ ] **Floating text polish** — `MISS` (серый) при dodge, `БЛОК` (синий) при thorns, красный/оранжевый крит vs белый удар, screen shake при ударе босса. Делает уже реализованные механики видимыми. ~2–3 часа, высокий ROI.
- [ ] **Флаги мобов** — механические архетипы в `mobs.js`: `shield` (поглощает N урона), `regen` (восстанавливает HP каждый тик), `armored` (DEF×1.5, уязвим к критам), `swift` (скорость×2, HP×0.5); боссы гарантированно 1–2 флага; иконки флагов в чипах BattleStrip. **Ключевая фича**: делает статы pierce/crit/deathblow/dodge тактически осмысленными.
- [ ] **Элиты (wave % 5)** — мини-боссы: HP×3, ATK×1.8, гарантированный дроп rare-предмета; стиль чипа с ⚡ в BattleStrip. Запускать после флагов мобов — синергия флагов + элиты.
