...
> Последнее обновление: 2026-05-07 (v1.9.0: главное меню + инфраструктура i18n)
...
```
src/
  main.js              (112 строки)  — точка входа, монтирование Phaser + UI + MainMenu
  i18n/
...
- **68 ручных классов depth 5** (план): все 68 имён + нарративы → **[DEPTH5.md](DEPTH5.md)**
- **~4000 генерируемых** (depth 6–10 после реализации depth 5, depth 5–10 сейчас): `generateDeepClasses()` при старте
...
Инфраструктура готова (`src/i18n/`). Расширение по итерациям:

- [x] **Итерация 1** — i18n система + главное меню полностью локализовано (v1.9.0)
- [ ] **Итерация 2** — HUD + BattleStrip + skill-zone (строки в JS-компонентах)
- [ ] **Итерация 3** — ClassTree + StatsPanel + модалки (class, prestige, settings)
- [ ] **Итерация 4** — inline-строки в index.html (stat names, labels, combat log)

Принцип: `t('key')` вызывается в JS-компонентах при рендере. Для статических HTML-строк — атрибут `data-i18n="key"` + проход `applyTranslations()` при смене языка.
...
- [x] **Кросс-ветковые способности** — 15 классов depth 2–4 получили уникальные механики из других веток: Убийца→deathblow, Ниндзя→pierce, Тень→lifesteal, Призрак→magicShield, Инквизитор→magicShield, Тёмный Судья→deathblow, Охотник на Ведьм→magicShield, Гладиатор→thorns, Охотник за Головами→deathblow, Пират→lifesteal, Шаман→dodge, Алхимик→poison, Тёмный Стрелок→poison, Аптекарь→poison, Отравленная Стрела→poison
...
- [x] **Яд (poison) — DoT механика** — стат `poison` (0–60%, тип B: только класс): шанс наложить яд при ударе. Отравленный моб получает `atk * 0.15` урона каждые 200мс (12 тиков), игнорируя броню. Не стакается, освежает таймер. Новые классы: `toxicologist` (d3), `poison_master` + `plague_doctor` (d4). Кросс-бонусы: Алхимик, Тёмный Стрелок, Аптекарь, Отравленная Стрела. Реализовано в v1.8.0.
...
- [ ] **Ручные классы depth 5** — заменить авто-генерацию вручную; план (68 классов с именами и нарративами) готов в **[DEPTH5.md](DEPTH5.md)**
...
- [ ] **Расширение дерева: 3-й ребёнок у выбранных классов** — точечное добавление, не у всех. Правило: 3-й ребёнок только там, где несёт новую уникальную механику, и имеет ту же глубину что и остальные два. Таблица запланированных расширений:
  ...
  |----------|-------|-------------------|---------------------|
  | `alchemist` | 2 | ~~Токсиколог~~ ✅ v1.8.0 | poison |
  | `berserker` | 2 | Воин Пламени | burn |
  | `sniper` | 2 | Шрапнельный Стрелок | splash |
  | `toxicologist` | 3 | ~~Мастер Ядов, Чумной Доктор~~ ✅ v1.8.0 | poison (глубже) |
