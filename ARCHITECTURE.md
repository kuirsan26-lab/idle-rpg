...
> Последнее обновление: 2026-05-11 (ачивки 20 шт., PP только из ачивок, пассивный powerMult скиллов, убран class-tree-panel → кнопка в HUD)
...
src/
  ...
  data/
    classes.js          — 148 ручных классов (depth 1–5, в т.ч. 20 престиж) + generateDeepClasses()
...
- **148 ручных классов** (depth 1–5): Новичок → Воин/Плут/Лучник/Маг → … → 68 именных depth-5 + 20 престиж-классов
...
- **⭐ depth-4 престиж (5 классов):** require два depth-3 из разных веток; имеют 2 ручных depth-5 ребёнка
- **⭐⭐ depth-5 престиж (5 классов):** require два depth-4 (включая ⭐ prestиж-4); seed для авто-генерации depth 6–10
- В `ClassTreeGraph.js`: пунктирные золотые рёбра к secondary `requires` родителям; ⭐/⭐⭐ стиль нод
- В `ClassTree.js`: mystery-нода показывает prestige-prefix; модалка — секция требований с ✓/✗
